...
public class ObjWriter {
    ...
    private static void writeVertices(FileWriter writer, ArrayList<Vector3> vertices) throws IOException {
    ...
    private static void writeTextureCoordinates(FileWriter writer, ArrayList<Vector2> textures) throws IOException {
        for (Vector2 uv : textures) {
            writer.write(String.format(Locale.US, "vt %.6f %.6f\n", uv.getX(), uv.getY()));
        }
        if (!textures.isEmpty()) writer.write("\n");
    }

    private static void writeNormals(FileWriter writer, ArrayList<Vector3> normals) throws IOException {
